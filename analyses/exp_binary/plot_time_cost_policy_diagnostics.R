...
matching_simulation_files <- function(lambda_value, alpha_value, beta_value, opportunity_value) {
  ...
  for (variant_file_segment in variant_file_segments) {
    pattern <- paste0(
      "^lambda_([^_]+)_alpha_([^_]+)_beta_([^_]+)_opportunity_([^_]+)_",
      "expansion_", expansion_decision_version, "_", variant_file_segment,
      "seed_([0-9]+)_", tree_file_label, "_", input_type, "\\.csv$"
    )
    matches <- regexec(pattern, basename(files))
    pieces <- regmatches(basename(files), matches)
    for (i in seq_along(pieces)) {
...
normalize_chosen_path_reward <- function(chosen_path_reward, path_rewards) {
  if (is.na(chosen_path_reward) || any(is.na(path_rewards))) {
    return(NA_real_)
  }
  min_value <- min(path_rewards)
  max_value <- max(path_rewards)
  denom <- max_value - min_value
  if (!is.finite(denom) || abs(denom) < 1e-12) {
    return(1)
  }
  (chosen_path_reward - min_value) / denom
}
...
build_trial_diagnostics <- function(dat) {
  ...
  trial_data$observations_before_stop <- vapply(seq_len(nrow(trial_data)), function(row_i) {
    vals <- suppressWarnings(as.numeric(trial_data[row_i, paste0("expanded_reward_t", reward_timesteps), drop = TRUE]))
    sum(!is.na(vals))
  }, integer(1))
  trial_data[
...
plot_parameter_legend <- function(include_optimal = TRUE) {
  labels <- color_legend_labels
  legend_cols <- unname(color_cols[color_levels])
  legend_pch <- rep(19, length(labels))
  legend_lty <- rep(1, length(labels))
  if (include_optimal) {
    labels <- c(labels, "optimal")
...
canonical_state_label <- function(path_states) {
  ...
  order_keys <- vapply(path_states, function(values) {
    paste(c(length(values), sprintf("%+.12f", values)), collapse = "|")
  }, character(1))
  path_states <- path_states[order(order_keys)]
  paste(
...
read_exact_table <- function(suffix) {
  path <- exact_file(suffix)
  if (!file.exists(path)) {
    warning(sprintf("Exact %s file not found at %s; optimal overlay will be skipped.", suffix, path))
    return(NULL)
  }
  read.csv(path, stringsAsFactors = FALSE)
}

exact_states <- read_exact_table("states")
exact_actions <- read_exact_table("actions")
exact_occupancy <- read_exact_table("occupancy")

requested_time_costs <- suppressWarnings(as.numeric(opportunity_values))
...
exact_p_continue_stop_prefer <- function(state_label, time_cost) {
  if (is.null(exact_actions_task)) {
    return(NA_real_)
  }
  state_actions <- exact_actions_task[
    exact_actions_task$state_label == state_label &
...
build_exact_trial_overlay <- function() {
  ...
  rows <- lapply(unique(requested_time_costs), simulate_exact_trials_for_cost)
...
build_exact_continue_summary <- function(feature_type) {
  ...
  feature_col <- if (feature_type == "best") "best_path_value" else "path_value_difference"
  split_keys <- interaction(dat$time_cost, dat$decision_timestep, dat[[feature_col]], drop = TRUE)
  summary_rows <- lapply(split(dat, split_keys), function(piece) {
    ...
    out[, c("time_cost", "decision_timestep", feature_col, "p_continue", "n"), drop = FALSE]
  })
...
build_exact_signed_difference_continue_summary <- function() {
  ...
  rows <- lapply(unique(requested_time_costs), build_exact_signed_difference_continue_summary_for_cost)
  rows <- rows[vapply(rows, nrow, integer(1)) > 0]
  if (length(rows) == 0) {
    return(data.frame())
  }
...
build_average_kl_reward_summary <- function(dat) {
  ...
  summary_data <- aggregate(
    cbind(kl_paid_total, chosen_path_reward, normalized_chosen_path_reward, observations_before_stop) ~ beta + opportunity,
    data = dat,
    FUN = mean
  )
  ...
  merge(summary_data, count_data, by = c("beta", "opportunity"))
}

build_exact_average_kl_reward_summary <- function(dat) {
  if (nrow(dat) == 0 || !"normalized_chosen_path_reward" %in% names(dat)) {
    return(data.frame())
  }
  summary_data <- aggregate(
    cbind(kl_paid_total, chosen_path_reward, normalized_chosen_path_reward, observations_before_stop) ~ opportunity,
    data = dat,
    FUN = mean
  )
  count_data <- aggregate(
    kl_paid_total ~ opportunity,
    data = dat,
    FUN = length
  )
  names(count_data)[names(count_data) == "kl_paid_total"] <- "n"
  merge(summary_data, count_data, by = "opportunity")
}
...
plot_average_summary_scatter <- function(
  ...
  pdf_path <- file.path(
    results_dir,
    sprintf(
      "%s_%s_lambda_%s_alpha_%s_beta_%s_opportunity_%s_expansion_%s_%s.pdf",
      file_prefix,
      input_type, lambda_arg, alpha_arg, beta_label, opportunity_label,
      expansion_label, tree_file_label
    )
  )
  pdf(pdf_path, width = 9.5, height = 5.5)
  old_par <- par(mar = c(4.5, 4.5, 1, 8))
  x_values <- summary_data[[x_col]]
  y_values <- summary_data[[y_col]]
  if (nrow(exact_summary) > 0 && all(c(x_col, y_col) %in% names(exact_summary))) {
    x_values <- c(x_values, exact_summary[[x_col]])
    y_values <- c(y_values, exact_summary[[y_col]])
  }
  plot(
    NA,
  ...
  points(
    ...
    cex = 1.35,
    col = mapply(series_color, summary_data$beta, summary_data$opportunity)
  )
  plot_parameter_legend(include_optimal = TRUE)
  par(old_par)
  dev.off()
...
plot_average_summary_scatter(
  ...
  file_prefix = "average_kl_paid_vs_average_normalized_chosen_path_reward"
)
plot_average_summary_scatter(
  average_kl_reward_summary,
  exact_average_kl_reward_summary,
...
plot_continue_summary <- function(summary_data, exact_summary, feature_col, xlab, file_prefix) {
  ...
  pdf_path <- file.path(
    ...
    sprintf(
      "%s_%s_lambda_%s_alpha_%s_beta_%s_opportunity_%s_expansion_%s_%s.pdf",
      file_prefix, input_type, lambda_arg, alpha_arg, beta_label, opportunity_label,
      expansion_label, tree_file_label
    )
  )
  ...
  n_cols <- min(3L, max(1L, length(timesteps)))
  n_rows <- ceiling(max(1L, length(timesteps)) / n_cols)
  pdf(pdf_path, width = max(9.5, 4.2 * n_cols), height = max(5.5, 4.2 * n_rows))
  old_par <- par(mfrow = c(n_rows, n_cols), mar = c(4.5, 4.5, 2.2, 8))
  x_values <- summary_data[[feature_col]]
  y_values <- summary_data$p_continue
  if (!is.null(exact_summary) && nrow(exact_summary) > 0 && feature_col %in% names(exact_summary)) {
  ...
  for (decision_timestep in timesteps) {
    plot(
      ...
      xlab = xlab,
      ylab = "P(continue)",
      main = sprintf("Decision t%d", decision_timestep),
      xaxt = "n"
    )
    axis(1, at = x_ticks)
    grid()
    ...
    if (!is.null(exact_summary) && nrow(exact_summary) > 0 && feature_col %in% names(exact_summary)) {
      for (time_cost in sort(unique(exact_summary$time_cost))) {
        piece <- exact_summary[
          exact_summary$decision_timestep == decision_timestep &
            abs(as.numeric(exact_summary$time_cost) - as.numeric(time_cost)) < 1e-8,
          ,
          drop = FALSE
        ]
        if (nrow(piece) == 0) {
