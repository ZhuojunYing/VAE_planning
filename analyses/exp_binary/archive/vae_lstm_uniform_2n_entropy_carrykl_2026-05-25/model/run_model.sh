...
if [ "$#" -lt 11 ] || [ "$#" -gt 13 ]; then
    echo "Usage: $0 lambda alpha beta model_dir sim_dir trial_n input_type seed train opportunity_cost tree_size [expansion_decision_version] [model_variant]"
...
python model/main.py "$lambda_string" "$alpha_string" "$beta_string" "$model_dir_name" "60" "$input_type" "$seed" "$tree_size" "$train" "" "$opportunity_cost_string" "$expansion_decision_version" "$model_variant"
