...
The solver assumes:
  ...
  * the only expansion cost is a fixed time/opportunity cost per observation;
...
path. For example, disjoint3x2 is represented as a multiset of three path
states, each path state being the sorted rewards observed on that path. This
keeps the exact state space small while preserving all information relevant to
...
TASKS = {
    ...
    "disjoint2x2": TaskSpec(
        name="disjoint2x2",
        num_paths=2,
        path_length=2,
        tree_size=4,
        tree_config="disjoint2x2",
    ),
    "disjoint3x2": TaskSpec(
        name="disjoint3x2",
        num_paths=3,
        path_length=2,
        tree_size=6,
        tree_config="disjoint3x2",
    ),
}
    ...
    def solve(self, state):
        ...
        for action in self.unique_observe_actions(state):
            ...
            for reward in self.rewards:
                ...
                next_state = replace_one_path_state(
                    state,
                    action["path_state"],
                    next_path_state,
                )
                expected_future_value += self.reward_prob * self.solve(next_state)["value"]
            q_value = -self.time_cost + expected_future_value
            actions.append({
                "kind": "observe",
    ...
    def state_rows(self):
        ...
        for state_id, state in enumerate(sorted(self.seen_states, key=state_label)):
            ...
            rows.append({
                ...
                "optimal_observe_path_states": json.dumps(
                    optimal_observe_path_states,
                    separators=(",", ":"),
                ),
        ...
        for state in sorted(self.seen_states, key=state_label):
            ...
            for action_index, action in enumerate(result["actions"]):
                rows.append({
                    ...
                    "observe_path_state": (
                        "" if action["path_state"] is None
                        else json_path_state(action["path_state"])
                    ),
            ...
            for state, state_mass in sorted(active.items(), key=lambda item: state_label(item[0])):
                ...
                for action in actions:
                    ...
                    for reward in self.rewards:
                        ...
                        next_active[next_state] = (
                            next_active.get(next_state, 0.0)
                            + transition_mass * self.reward_prob
                        )
    ...
    for task_name in task_names:
        ...
        for time_cost in time_costs:
            ...
            print(
                ...
                f"V0={solver.solve(empty_state(task))['value']:.8g}, "
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Calculate exact perfect-memory optimal policies for default2, "
            "bandit3, bandit4, disjoint2x2, and disjoint3x2 with time cost only."
        )
    )
    parser.add_argument(
        "--task",
        nargs="+",
        default=["all"],
        help="Task names or aliases. Use 'all' for every supported task.",
    )
    parser.add_argument(
        "--time-costs",
        default="0.0,0.01,0.05,0.1,0.2,0.5,1.0",
        help="Comma-separated per-observation time/opportunity costs.",
    )
    parser.add_argument(
        "--input-type",
        choices=("uniform", "binary"),
        default="uniform",
        help="Reward alphabet. 'uniform' uses the eight nonzero rewards by default.",
    )
    parser.add_argument(
        "--reward-values",
        default=",".join(fmt_num(value) for value in DEFAULT_REWARDS),
        help="Comma-separated reward alphabet for --input-type uniform.",
    )
    parser.add_argument(
        "--no-normalize-reward",
        action="store_true",
        help=(
            "Use raw path rewards. By default, path rewards are divided by the "
            "task's exact E[max complete path reward], matching model.py's "
            "expansion return target scale."
        ),
    )
    parser.add_argument(
        "--min-observations-before-stop",
        type=int,
        default=0,
        help=(
            "Minimum observations before stopping is legal. Use 1 to match the "
            "decoder expansion condition; default 0 allows initial stopping."
        ),
    )
    parser.add_argument(
        "--tie-mode",
        ...
        help="How to assign probabilities when several optimal actions tie.",
    )
    parser.add_argument(
        "--tie-tol",
        type=float,
        default=1e-10,
        help="Absolute tolerance for identifying value ties.",
    )
    parser.add_argument(
        "--output-dir",
        default="analyses/exp_binary/results/exact_time_cost",
        help="Directory for CSV outputs.",
    )
    parser.add_argument(
        "--output-prefix",
        default="exact_time_cost",
        help="Filename prefix for the four output CSVs.",
    )
    return parser
