...
if ! {
    ...
    echo "Pass --sigma-list 0,0.25,0.5 in extra_jax_args to run one local job per sigma value."
...
echo "Running JAX grid locally and sequentially."

commands_file=$(mktemp "${TMPDIR:-/tmp}/jax_grid_local.XXXXXX")
...
python - "$beta_arg" "$beta_max" "$beta_steps" "$alpha_min" "$alpha_max" "$alpha_steps" \
    ...
    "$n_sim_trials" "${extra_args[@]}" > "$commands_file" <<'PY'
import itertools
import os
import shlex
import sys
...
(
    ...
    num_updates, num_envs, n_sim_trials, *extra_args
) = sys.argv[1:]
...
print(f"Local JAX grid contains {len(jobs)} job(s).", file=sys.stderr)
print(
    "Each job uses steps_per_epoch = "
    f"max(num_updates * num_envs * num_steps // 120, num_envs * num_steps) = {steps_per_epoch}.",
    file=sys.stderr,
)
for cmd in jobs:
